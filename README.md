...
- 📜 **历史记录**：支持查询服务器端历史记录，可按模式、状态、时间范围过滤，支持分页浏览和任务详情查看
   ...
   - **多选功能**：支持多选历史记录，选中状态不受翻页影响
   - **批量导出**：支持 JSON、XML、YAML 三种格式，导出后自动下载到系统下载目录
   - **操作提示**：删除或导出后显示成功/失败提示
- 🛑 **任务取消**：检测过程中可随时取消正在执行的任务，取消后保留已处理的结果
- ⚙️ **设置页面**：支持快捷键绑定、历史记录保留期限设置、本地数据管理等功能
- ⌨️ **全局快捷键**：支持自定义快捷键触发检测、取消任务、清空重置操作，配置持久化保存
- 🎨 **专业 UI 设计**：使用 MUI 和 Tremor 组件库优化视觉效果，提升专业感和用户体验
- 📝 **历史记录页码编辑**：分页组件支持直接点击编辑页码，输入后立即跳转，支持数字验证和快速导航
...
│   │   ├── stats.ts          # 统计计算工具
│   │   └── exportUtils.ts    # 导出工具（JSON/XML/YAML）
│   ├── store/                    # Zustand Store
│   │   ├── index.ts              # Store 统一导出
...
前端支持将历史记录导出为文件，方便存档和分析：

| 功能 | 说明 |
|------|------|
| **导出格式** | JSON（默认）、XML、YAML 三种可选 |
| **导出内容** | 任务信息 + 每张图片的详细检测结果 |
| **导出范围** | 用户勾选的历史记录 |
| **保存位置** | 系统默认下载目录 |
| **文件名** | 默认格式：`历史记录_YYYYMMDD_HHMMSS.格式`，支持自定义 |
...
- **分页浏览**：每页显示 9 条记录（有选中项时 8 条），支持翻页，可直接点击页码编辑快速跳转
- **多选功能**：每条记录前有复选框，支持多选，选中状态不受翻页影响
- **批量操作**：有选中项时显示操作栏，支持全选、取消全选、导出
- **导出功能**：支持 JSON、XML、YAML 三种格式，导出对话框可自定义文件名，文件自动下载到系统下载目录
- **详情查看**：点击任意任务行可查看详细信息和检测结果
- **删除功能**：支持删除单条历史记录（API Key 认证即可），删除后显示成功/失败提示
- **数据存储**：所有历史记录存储在服务器端，便于跨设备访问
- **统计信息**：顶部显示统计卡片，根据当前筛选条件动态计算（成功率、平均耗时等）
- **自动更新**：检测完成后自动添加到历史记录列表，无需手动刷新
- **操作提示**：导出或删除后显示 Snackbar 提示，成功为绿色，失败为红色，4 秒后自动消失
